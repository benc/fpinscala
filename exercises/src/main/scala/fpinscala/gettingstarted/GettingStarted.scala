...
object MyModule {
  ...
  def fib(n: Int): Int = {
    ...
    def go(n: Int, first: Int, second: Int): Int =
      if (n == 0) first // niet nodig om hier nog eens de boel op te tellen, hier geef je de laatst berekende waarde terug
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A, A) => Boolean): Boolean = {
    ...
    def go(n: Int): Boolean =
      if (n >= as.length - 1) true // einde van array bereikt
  ...
  def curry[A, B, C](f: (A, B) => C): A => (B => C) = {
    // (a: A) => (b: B) => f(a, b) // meer verbose, je mag het type achterwege laten, scala kan dit zelf uitzoeken
    // f.curry
    a => b => f(a, b)
  }
  ...
  */
  ...
  def compose[A, B, C](f: B => C, g: A => B): A => C = {
    // f compose g
    // f andThen g
    // begin met a, kijk of er een ft is die a aanpakt en bouw van daar uit verder
    a => f(g(a))
  }
}
